...
FROM
coupon AS c
left join (select coupon_id, count(coupon_condition_id) as total_condition from coupon_condition where coupon_id=$couponId) as cc on cc.coupon_id=c.coupon_id
left join (select coupon_id, count(coupon_target_id) as total_target from coupon_target where coupon_id=$couponId) as ct on ct.coupon_id=c.coupon_id
left join (select coupon_id, count(coupon_requirement_id) as total_requirement from coupon_requirement where coupon_id=$couponId) as cr on cr.coupon_id=c.coupon_id
left join (select coupon_id, count(coupon_reward_id) as total_reward from coupon_reward where coupon_id=$couponId) as crw on crw.coupon_id=c.coupon_id
left join (select coupon_id, count(coupon_subject_id) as total_subject from coupon_subject where coupon_id=$couponId) as cs on cs.coupon_id=c.coupon_id
