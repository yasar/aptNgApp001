<?php
/**
 * Created by IntelliJ IDEA.
 * User: unal
 * Date: 21.01.2016
 * Time: 11:28
 */


namespace BYRWEB\app001\saleitemPackage;




use BYRWEB\base\ADbObject;
use BYRWEB\base\IDbObject;


class SaleitemPackage extends ADbObject implements IDbObject
{
	/**
	 *
	 */
	public
	function __construct()
	{
		parent::__construct();
		$this->setRecordObject(new SaleitemPackageRecord());
	}
	
	//    /**
	//     * @param $id
	//     * @return SaleitemPackageRecord
	//     */
	//    public function get($id)
	//    {
	//        /**
	//         * @var SaleitemPackageRecord
	//         */
	//        return parent::get($id);
	////
	//    }
	//
	//    /**
	//     * @param $data
	//     * @return bool|SaleitemPackageRecord
	//     */
	//    public function add($data, $updateOnDuplicate = false)
	//    {
	//        return parent::add($data, 'qty=qty+1');
	//    }
	//
	//    /**
	//     * @param $data
	//     * @return bool|SaleitemPackageRecord
	//     */
	//    public function update($data)
	//    {
	//
	//        return parent::update($data);
	//    }
	//
	//    /**
	//     * @param null $filter
	//     * @param null $keyword
	//     * @param null $limit
	//     * @param array $order
	//     * @return array
	//     */
	//    public function find($filter = null, $keyword = null, $limit = null, array $order = [], $selectOnly = null, $lookIn = null)
	//    {
	//        return parent::find($filter, $keyword, $limit, $order, $selectOnly, $lookIn); // TODO: Change the autogenerated stub
	//    }
	
	
}