...
INNER JOIN `app900.sale` AS s ON s.sale_id = s_i.sale_id ";
    ...
    public function find($filter = null, $keyword = null, $limit = null, array $order = [], $selectOnly = null, $lookIn = null)
    {
        ...
        switch ($for) {
            case 'saleInvoice':
                $this->getRecordObject()->setSqlQuery(new SqlObject(file_get_contents(__DIR__ . '/sql/saleInvoice.sql')), $this->for);
                $result = parent::find($filter, $keyword, $limit, $order, $selectOnly, $lookIn);
                break;
            case 'purchase':
            case 'purchaseInvoice':
                $this->getRecordObject()->setSqlQuery(new SqlObject(file_get_contents(__DIR__ . '/sql/purchaseInvoice.sql')), $this->for);
                $result = parent::find($filter, $keyword, $limit, $order, $selectOnly, $lookIn);
                break;
            case 'cashSale':
                $this->getRecordObject()->setSqlQuery(new SqlObject(file_get_contents(__DIR__ . '/sql/cashSale.sql')), $this->for);
                $result = parent::find($filter, $keyword, $limit, $order, $selectOnly, $lookIn);
                break;
            case 'shopping_cart':
            case 'browse':
