...
use BYRWEB\app001\sale\Sale;
use BYRWEB\app001\sale\SaleRecord;
use BYRWEB\app001\saleitem\Saleitem;
use BYRWEB\app001\saleitemPackage\SaleitemPackage;
use BYRWEB\app001\saleSaleItem\SaleSaleItem;
...
//            self::processCurrent($post, $inspector, $transaction_id);
    ...
    private static function init(&$post)
    {
    ...
    private static function processStock($sale_items)
    {
        /**
    ...
    private static function processPackageSaleitemsStock($sale_item)
    {
...
//        $newDate = new WTDate();
        ...
        $transactionRecord = $transaction->add($data);

        return $transactionRecord->transaction_id;
    }
    ...
    public static function getAccountAndTxTypeByPaymentType($payment_type_id)
    {
        ...
        foreach ($payment_types as $payment_type) {
            ...
            if (intval($payment_type->type_id) !== intval($payment_type_id)) {
                continue;
            }
        ...
        return [
            'account_id'           => $special_account->account_id,
            'tx_type'              => $tx_type,
    ...
    private static function processSale($post, $inspector, $transaction_id)
    {
    ...
    private static function processCurrentSplits($transaction_id)
    {
    ...
    private static function processInvoice($post)
    {
    ...
    private static function processPoints($inspector, $splits)
    {
        ...
        foreach ($splits as $split) {
            ...
            $pointRecord->money_spent    = $pointRecord->money_spent + $pointRecord->points_spent * $pointValueRecord->value;
            $pointRecord->sale_id        = self::$saleRecord->sale_id;
            $pointRecord->point_value_id = $pointValueRecord->point_value_id;
            $pointRecord->card_id        = 1;
            $pointRecord->add();
    ...
    private static function addClientPackageItem($sale_items)
    {
        $package_items = [];
    ...
    private static function processCurrent($post, $inspector, $transaction_id)
    {
        $arr = [
            ['debt' => $inspector->new_total['grand'], 'credit' => null, 'description' => 'Satış'],
